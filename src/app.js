...
(function () {
  ...
  function freshState() {
    return {
      ...
      scanLog: [],      // {t, input, result, key} — drives Undo (scans only)
      activityLog: [],  // {t, kind, key, delta, count, raw} — chronological audit of every scan & manual adjust
  ...
  // These parse-time structures are large (the full inventory across every
  // location can be ~10 MB on a real catalog) and are only ever used BEFORE a
  // count begins (location picker + building count lines). Persisting them
  // would blow the browser's ~5 MB localStorage quota — saves would silently
  // fail and resume would break — and would make every scan re-serialize
  // megabytes. So they're excluded from the saved session; an in-progress or
  // finished count restores fully from the remaining (small) state.
  var PERSIST_OMIT = { inventoryByLocation: 1, products: 1, productsBySku: 1 };
  ...
  function loadState() {
    try {
      ...
      // Merge over fresh defaults so omitted (un-persisted) structures come
      // back as empty objects rather than undefined.
  ...
  function saveState() {
    try {
      var toSave = {};
      Object.keys(state).forEach(function (k) { if (!PERSIST_OMIT[k]) toSave[k] = state[k]; });
      ...
      if (!quotaWarned) {
        quotaWarned = true;
        toast('Warning: this count is too large to auto-save for crash recovery. It will still work, but export before closing the tab.', 'bad');
      }
    }
  }
  ...
  // Round money to whole cents so stored/exported dollar figures are exact
  // (no floating-point tails like -187731.14000000007 that could confuse an
  // accountant summing the column). The +EPSILON avoids 0.5-cent edge drift.
  function round2(n) {
  ...
  function selectLocation(location) {
    // The full inventory isn't persisted (see PERSIST_OMIT), so after a reload
    // on the pre-count screens there's nothing to build from — send the user
    // back to re-upload rather than building an empty count.
    if (!state.inventoryByLocation || !state.inventoryByLocation[location]) {
      toast('Please re-upload your Products and Inventory CSVs to start a count here.', 'warn');
      state.screen = 'start';
      saveState();
      render();
      return;
    ...
    // Reset the counting-screen view state for the new count so a tab that
    // only exists for some locations (e.g. "No barcode") can't stay selected
    // and show an empty, tab-less list at the next location.
    countingTab = 'all';
    countingSearch = '';
    saveState();
    render();
  ...
  // Append-only audit of every scan and manual adjustment, shown in the
  // Activity log tab. Bounded so a very large count can't blow the storage
  // quota (which would silently break save/resume).
  var ACTIVITY_CAP = 5000;
  function logActivity(kind, opts) {
    opts = opts || {};
    state.activityLog.push({
      t: nowStr(),
      kind: kind, // 'match' | 'extra' | 'manual' | 'unknown' | 'conflict' | 'undo'
      key: opts.key || null,
      delta: opts.delta || 0,
    ...
    if (state.activityLog.length > ACTIVITY_CAP) {
      state.activityLog.splice(0, state.activityLog.length - ACTIVITY_CAP);
    }
  }
  ...
  function processScan(rawValue) {
    ...
    if (key) {
      ...
      logActivity('match', { key: key, delta: 1, count: line.counted });
      ...
      logActivity('extra', { key: extraKey, delta: 1, count: el.counted });
  ...
  function manualAdjust(key, delta, isExtra) {
    ...
    if (bumpLine(lines, key, delta)) {
      logActivity('manual', { key: key, delta: delta, count: lines[key].counted });
    }
    saveState();
    renderCountingStatsTabs();
    renderCountingLists();
  ...
  function undoLastScan() {
    ...
    if (entry.result === 'ok') {
      ...
      count = state.countLines[entry.key] ? state.countLines[entry.key].counted : null;
    } else if (entry.result === 'extra') {
      bumpLine(state.extraLines, entry.key, -1);
      count = state.extraLines[entry.key] ? state.extraLines[entry.key].counted : null;
    ...
    logActivity('undo', { key: entry.key, count: count, raw: entry.input });
    saveState();
  ...
  function buildReportRows() {
    ...
    all.forEach(function (l) {
      ...
      l.costImpact = round2(l.diff * cost);
    });
  ...
  function reportTotals(rows) {
    ...
    // Round the accumulated totals to cents so summing many rows can't leave a
    // floating-point tail in the displayed or exported figures.
    t.overageDollars = round2(t.overageDollars);
    t.shortageDollars = round2(t.shortageDollars);
    t.netDollars = round2(t.overageDollars - t.shortageDollars);
    return t;
  }
  ...
  function renderLocation() {
    ...
    html += stepper(1);
    var hasInventory = state.inventoryByLocation && Object.keys(state.inventoryByLocation).length > 0;
    if (!hasInventory) {
      // Happens only after a reload landed here (the full inventory isn't
      // persisted). The CSVs must be re-uploaded to pick a location.
      html += '<div class="banner warn">Your uploaded files aren\'t loaded in memory anymore (the page was reloaded). Please re-upload your Products and Inventory CSVs to start a new count.</div>';
      html += '<button id="btn-back-start" class="primary">&larr; Back to upload</button>';
      root.innerHTML = html;
      document.getElementById('btn-back-start').onclick = function () { state.screen = 'start'; saveState(); render(); };
      return;
    }
  ...
  function renderCountingStatsTabs() {
    ...
    // Items with no barcode can't be scanned — they must be counted manually,
    // so give counters a dedicated tab to work through exactly those.
    var noBarcodeLines = lines.filter(function (l) { return l.noBarcode; });
    var noBarcodeUncounted = noBarcodeLines.filter(function (l) { return l.counted === 0; }).length;
    ...
    if (noBarcodeLines.length) {
      html += tabBtn('nobarcode', 'No barcode — count manually (' + noBarcodeUncounted + ' left of ' + noBarcodeLines.length + ')');
    }
    ...
    html += tabBtn('activity', 'Activity log (' + state.activityLog.length + ')');
    html += '</div>';
  ...
  function activityLabelFor(e) {
    if (e.key && state.countLines[e.key]) {
      var l = state.countLines[e.key];
      return esc(l.title) + (l.variantLabel ? ' — ' + esc(l.variantLabel) : '') + ' <span class="muted">(' + esc(l.sku) + ')</span>';
    }
    if (e.key && state.extraLines[e.key]) {
      var x = state.extraLines[e.key];
      return esc(x.title) + (x.variantLabel ? ' — ' + esc(x.variantLabel) : '') + ' <span class="muted">(' + esc(x.sku) + ', unexpected)</span>';
    }
    if (e.raw) return '<span class="muted">barcode ' + esc(e.raw) + '</span>';
    return '<span class="muted">—</span>';
  }

  function activityActionFor(e) {
    switch (e.kind) {
      case 'match': return '<span style="color:var(--good)">Scan +1</span>';
      case 'extra': return '<span style="color:var(--warn)">Scan +1 (unexpected)</span>';
      case 'manual': return (e.delta >= 0 ? '<span style="color:var(--good)">Manual +' + e.delta + '</span>' : '<span style="color:var(--bad)">Manual ' + e.delta + '</span>');
      case 'unknown': return '<span style="color:var(--bad)">Unrecognized scan</span>';
      case 'conflict': return '<span style="color:var(--bad)">Ambiguous barcode</span>';
      case 'undo': return '<span style="color:var(--muted)">Undo</span>';
  ...
  function renderActivityLog() {
    ...
    if (!log.length) return '<p class="muted">No activity yet. Scans and manual +/- adjustments will be recorded here, newest first.</p>';

    var q = countingSearch.trim().toLowerCase();
    // Show newest first. Search filters by item title/SKU/variant or scanned value.
    var entries = log.slice().reverse();
    if (q) {
      entries = entries.filter(function (e) {
        var l = (e.key && (state.countLines[e.key] || state.extraLines[e.key])) || null;
        var hay = (l ? (l.title + ' ' + (l.variantLabel || '') + ' ' + l.sku) : (e.raw || '')).toLowerCase();
    ...
    var html = '<table><thead><tr><th>Time</th><th>Action</th><th>Item</th><th class="num">New count</th></tr></thead><tbody>';
    shown.forEach(function (e) {
      html += '<tr>' +
        '<td class="muted" style="white-space:nowrap;">' + esc((e.t || '').replace(/^.*,\s*/, '')) + '</td>' +
        '<td style="white-space:nowrap;">' + activityActionFor(e) + '</td>' +
        '<td>' + activityLabelFor(e) + '</td>' +
        '<td class="num">' + (e.count === null || e.count === undefined ? '' : e.count) + '</td>' +
        '</tr>';
    });
    html += '</tbody></table>';
    if (entries.length > maxRows) html += '<p class="muted">Showing the ' + maxRows + ' most recent of ' + entries.length + ' entries — use search to narrow down.</p>';
    return html;
  }
  ...
  function renderCountingLists() {
    ...
    if (countingTab === 'activity') {
      wrap.innerHTML = renderActivityLog();
      return;
    }
    ...
    if (isExtraTab) {
      ...
      lines = Object.values(state.countLines).filter(function (l) { return l.counted === 0; });
    } else if (countingTab === 'nobarcode') {
      lines = Object.values(state.countLines).filter(function (l) { return l.noBarcode; });
    } else {
    ...
    // Stable, count-independent order: items never move as they're counted,
    // so the +/- buttons stay under the user's cursor (no accidental presses).
    // Recent activity is tracked in the Activity log tab instead.
