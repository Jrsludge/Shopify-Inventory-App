...
(function () {
  ...
  function freshState() {
    return {
      ...
      products: [],          // [{sku, barcode, title, variantLabel, costPerItem, key}]
      productByKey: {},      // "sku||variant" -> product record (the precise lookup)
      barcodeToRecord: {},   // barcode -> product record (single owner)
      barcodeConflicts: {},  // barcode -> [record,...]  (2+ distinct variants sharing a barcode)
  ...
  // Some suppliers only distinguish e.g. King vs Cal King by physical
  // barcode, not by SKU, so two genuinely different variants can share one
  // SKU string in Shopify. Every count line is therefore identified by
  // SKU + variant (not SKU alone), so those variants get separate expected
  // quantities, separate tallies, and don't clobber each other. The variant
  // portion is normalized (trim/lowercase) since it's compared across two
  // different Shopify export types that may format it slightly differently.
  function lineKey(sku, variantLabel) {
    return sku + '||' + (variantLabel || '').toLowerCase().trim();
  }
  ...
  function processProductsRows(rows) {
    ...
    var skuVariantLabels = {}; // sku -> distinct variant labels seen, to spot shared-SKU variants
    ...
    rows.forEach(function (row) {
      ...
      if (!skuVariantLabels[sku]) skuVariantLabels[sku] = [];
      if (skuVariantLabels[sku].indexOf(variantLabel) === -1) skuVariantLabels[sku].push(variantLabel);
      ...
      productByKey[key] = record;

      if (barcode && !isUnsafeKey(barcode)) {
        if (barcodeConflicts[barcode]) {
          if (barcodeConflicts[barcode].every(function (r) { return r.key !== key; })) barcodeConflicts[barcode].push(record);
        } else if (barcodeToRecord[barcode] && barcodeToRecord[barcode].key !== key) {
    ...
    var sharedSkus = Object.keys(skuVariantLabels)
      .filter(function (sku) { return skuVariantLabels[sku].length > 1; })
      .map(function (sku) { return { sku: sku, variantLabels: skuVariantLabels[sku] }; });
  ...
  function computeMissingFromProducts() {
    ...
    Object.keys(state.inventoryByLocation).forEach(function (loc) {
      ...
      Object.keys(m).forEach(function (key) {
        if (!m[key].notStocked && !state.productByKey[key]) missing[m[key].sku] = true;
      });
    });
  ...
  function handleProductsFile(file) {
    parseCsvFile(file, function (err, rows) {
      ...
      state.productByKey = result.productByKey;
  ...
  function selectLocation(location) {
    ...
    Object.keys(expected).forEach(function (key) {
      ...
      var product = state.productByKey[key];
      // Fall back to the Inventory file's own title/variant (not just a
      // generic placeholder) when this SKU+variant has no Products match.
      ...
      countLines[key] = {
        ...
        variantLabel: product ? product.variantLabel : entry.variantLabel,
  ...
  function processScan(rawValue) {
    ...
    if (state.countLines[key]) {
      ...
      addOrBumpExtra(key, record);
      entry.result = 'extra';
      var el = state.extraLines[key];
